...
type ApiItem = Omit<QuestionSearchResult, "dateDepot"> & {
  dateDepot: string | null;
};

const PAGE_SIZE = 5;

export default function QuestionsLoadMore({
  query,
  legislature,
  alreadyShown,
  total,
  ...
  const [items, setItems] = React.useState<QuestionSearchResult[]>([]);
  ...
  const loadMore = async () => {
    setLoading(true);
    setError(null);
    trackEvent("charger-plus", { section: "recherche-questions" });
    try {
      ...
      const parsed: QuestionSearchResult[] = data.items
        .map((q) => ({
          ...q,
          dateDepot: q.dateDepot ? new Date(q.dateDepot) : null,
        }))
        .filter((q) => {
          if (seenUids.current.has(q.uid)) return false;
          seenUids.current.add(q.uid);
          return true;
        });
      ...
      {remaining > 0 && (
        <Box sx={{ display: "flex", justifyContent: "center", mt: 2 }}>
          ...
          >
            {loading
              ? "Chargement…"
              : `Voir plus de questions (${remaining} restante${remaining > 1 ? "s" : ""})`}
          </Button>
        </Box>
      )}
      {error && (
